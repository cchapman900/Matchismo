...
#define CORNER_RADIUS_PERCENT 0.094

- (void)drawRect:(CGRect)rect
{
    UIBezierPath *roundedRect = [UIBezierPath bezierPathWithRoundedRect:self.bounds cornerRadius:self.bounds.size.width * CORNER_RADIUS_PERCENT];
    ...
    if (self.faceUp) {
        ...
        if (faceImage) {
            CGRect imageRect = CGRectInset(self.bounds,
                                           ...
                                           self.bounds.size.height*(1.0 - self.faceCardScaleFactor)*0.878);
        ...
        [[UIImage imageNamed:@"PlayingCardBack.png"] drawInRect:self.bounds];
    }
}
                 ...
                 upsideDown:(BOOL)upsideDown
{
    ...
    CGPoint pipOrgin = CGPointMake(middle.x-pipSize.width/2.0-hoffset*self.bounds.size.width,
...
#define CORNER_FONT_SIZE 0.16
#define CORNER_TEXT_INSET 0.0125
-(void)drawCorners
{
    NSMutableParagraphStyle *paragraphStyle = [[NSMutableParagraphStyle alloc] init];
    paragraphStyle.alignment = NSTextAlignmentCenter;
    
    UIFont *cornerFont = [UIFont fontWithName:@"HelveticaNeue-Light" size:self.bounds.size.width * CORNER_FONT_SIZE];
    ...
    CGRect textBounds;
    textBounds.origin = CGPointMake(self.bounds.size.width * CORNER_TEXT_INSET, self.bounds.size.width * CORNER_TEXT_INSET);
